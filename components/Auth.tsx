...
import { createUserWithEmailAndPassword, signInWithEmailAndPassword, setPersistence, browserLocalPersistence, signInAnonymously, signOut } from 'firebase/auth';
    ...
    if (view === 'LOGIN') {
      ...
      try {
          ...
          if (!appUser) {
              ...
              } catch (e) { console.error("Sync failed, proceeding anyway", e); }
          }
      ...
      try {
        ...
        const adminEmail = settings?.adminEmail || ADMIN_EMAIL;
        ...
        if (!showAdminVerify) {
            ...
            if (adminAuthCode === settings?.adminCode) {
                try {
                    ...
                    // --- CHANGED: Use Email Auth instead of Anonymous for Admin ---
                    // This ensures the Admin User has a STABLE UID and proper identity for Security Rules
                    let uid = '';
                    
                    // Generate a consistent password for Firebase Auth based on the Admin Code
                    // Must be at least 6 chars. Pad if necessary.
                    const firebasePwd = adminAuthCode.length < 6 ? adminAuthCode.padEnd(6, '0') : adminAuthCode;
                    
                    try {
                        // 1. Try Login
                        const cred = await signInWithEmailAndPassword(auth, formData.email, firebasePwd);
                        uid = cred.user.uid;
                    } catch (loginErr: any) {
                         if (loginErr.code === 'auth/user-not-found' || loginErr.code === 'auth/invalid-credential') {
                             // 2. Create Admin Account if missing
                             const cred = await createUserWithEmailAndPassword(auth, formData.email, firebasePwd);
                             uid = cred.user.uid;
                         } else if (loginErr.code === 'auth/wrong-password') {
                             // Fatal Error: Password mismatch (Admin code changed?)
                             // Fallback to Anonymous to at least let them in locally? 
                             // No, anonymous causes Permission Denied.
                             throw new Error("Admin Password Mismatch (Firebase). Code changed?");
                         } else {
                             throw loginErr;
                         }
                    }
                    ...
                    // We catch the error here to show a specific alert if rules block creation
                    try {
                        await saveUserToLive(adminUser);
                        logActivity("ADMIN_LOGIN", "Admin Access Granted");
                        onLogin(adminUser);
                    } catch (syncErr: any) {
                        console.error("Admin Sync Error:", syncErr);
                        // If rules block the write, we alert the user but maybe let them in locally?
                        // If we let them in locally, Dashboard will fail to load users.
                        setError(`Server Rejected Admin Role.\n\nError: ${syncErr.code || syncErr.message}\n\nPlease check Database Rules or Internet.`);
                    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 font-sans py-10">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-slate-200 relative overflow-hidden">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 text-sm font-bold p-4 rounded-xl mb-6 border border-red-100 flex items-start gap-2 animate-in slide-in-from-top-2 whitespace-pre-wrap">
